...
func _register_ui_core() -> void:
	...
	_merge_joy_btn(&"ui_left", JoyButton.DPAD_LEFT)
	_merge_axis(&"ui_left", JoyAxis.LEFT_X, -1.0)

	_action_ensure(&"ui_right")
	_merge_key(&"ui_right", KEY_RIGHT)
	_merge_joy_btn(&"ui_right", JoyButton.DPAD_RIGHT)
	_merge_axis(&"ui_right", JoyAxis.LEFT_X, 1.0)

	_action_ensure(&"ui_up")
	_merge_key(&"ui_up", KEY_UP)
	_merge_joy_btn(&"ui_up", JoyButton.DPAD_UP)
	_merge_axis(&"ui_up", JoyAxis.LEFT_Y, -1.0)

	_action_ensure(&"ui_down")
	_merge_key(&"ui_down", KEY_DOWN)
	_merge_joy_btn(&"ui_down", JoyButton.DPAD_DOWN)
	_merge_axis(&"ui_down", JoyAxis.LEFT_Y, 1.0)

	_action_ensure(&"ui_accept")
	_merge_key(&"ui_accept", KEY_ENTER)
	_merge_key(&"ui_accept", KEY_KP_ENTER)
	_merge_key(&"ui_accept", KEY_SPACE)
	_merge_joy_btn(&"ui_accept", JoyButton.A)

	_action_ensure(&"ui_cancel")
	_merge_key(&"ui_cancel", KEY_ESCAPE)
	_merge_joy_btn(&"ui_cancel", JoyButton.B)
...
func _register_menu_tabs() -> void:
	...
	_merge_joy_btn(&"menu_tab_prev", JoyButton.LEFT_SHOULDER)

	_action_ensure(&"menu_tab_next")
	_merge_key(&"menu_tab_next", KEY_E)
	_merge_joy_btn(&"menu_tab_next", JoyButton.RIGHT_SHOULDER)
...
func _merge_joy_btn(action: StringName, button: JoyButton) -> void:
...
func _action_has_joy_btn(action: StringName, button: JoyButton) -> bool:
	for ev in InputMap.action_get_events(action):
...
func _merge_axis(action: StringName, axis: JoyAxis, sign: float) -> void:
...
func _action_has_axis(action: StringName, axis: JoyAxis, sign: float) -> bool:
	for ev in InputMap.action_get_events(action):
